...
void cargarRep(int zonas[][4], unsigned f, unsigned c) //que es f? Filas
{
    int vehiculo;
    rep repartidor;
    ...
    while (vehiculo != 0)
    {
        ...
        case 1:
        {
            FILE *f = fopen("RepAuto.dat", "ab");   //lo que se hace por cada case podria estar en una funcion
            if (f == NULL)                          //para no repetir codigo
...
void pasarAVector(int zonas[][4], unsigned f, unsigned c) //pasa los datos del archivo a un vector con n posiciones, siendo n la cantidad de repartidores por zona (de todos los vehiculos)
{
    ...
    {  //en lugar de guardar los repartidores en un archivo, pasar el archivo a un vector
       //ordenarlo y depues reescribir el archivo
       //se podria al ingresar los repartidores haberlos guardado en un vector
       //ordenarlo y despues generar el archivo

#include <iostream>
using namespace std;

struct Examen
{
    int fecha, cod, nota;
};

struct NodoSub
{
    Examen info;
    NodoSub*sig;
};

struct Alumno
{
    int dni;
    string nombre;
    NodoSub* listaExam;
};

struct Nodo
{
    Alumno info;
    Nodo*sig;
};


void mostrarNotas(Nodo*lista);
Nodo *buscarInsertar(Nodo* &lista, Alumno a);
void generar(Nodo*&lista);
void insertar(NodoSub* &lista, Examen ex);

int main()
{
    Nodo*listaAlumnos = NULL;
    generar(listaAlumnos);
    mostrarNotas(listaAlumnos);

    return 0;
}

void generar(Nodo*&lista)
{
    Alumno al;
    Examen ex;
    Nodo*p;
    unsigned dni, codM, fechaEx, nota;
    string nom;
    cout<<"DNI: ";
    cin>>dni;
    while(dni!=0)
    {
        cout<<"Nombre: ";
        cin>>nom;
        cout<<"Fecha examen: ";
        cin>>fechaEx;
        cout<<"Materia rendida: ";
        cin>>codM;
        cout<<"Nota: ";
        cin>>nota;

        al.dni=dni;
        al.nombre=nom;
        al.listaExam=NULL;
        Nodo*p=buscarInsertar(lista, al);
        ex.cod=codM;
        ex.fecha=fechaEx;
        ex.nota=nota;
        insertar(p->info.listaExam, ex);

        cout<<"DNI: ";
        cin>>dni;
    }
}

Nodo *buscarInsertar(Nodo* &lista, Alumno a) 
{   
    Nodo *l, *ant;
    l = lista;
    while(l != NULL && l->info.dni < a.dni)
    {
        ant = l;
        l = l->sig;
    }
    if(l == NULL || l->info.dni!=a.dni) 
    {
        Nodo *n = new Nodo;
        n->info = a;
        n->sig = l;
        if(l != lista)
            ant->sig = n;
        else
            lista = n;
        return n;
    }
    else
        return l;
}

void insertar(NodoSub* &lista, Examen ex)
{
    NodoSub *n, *l, *ant;
    n = new NodoSub;
    n->info = ex;
    l = lista;
    while(l != NULL && l->info.fecha < ex.fecha)
    {
        ant = l;
        l = l->sig;
    }
    n->sig = l;
    if(l != lista)
        ant->sig = n;
    else
        lista = n;
}

void mostrarNotas(Nodo*lista)
{
    NodoSub*p;
    int suma, cont;
    while(lista != NULL)
    {
        suma=cont=0;
        p=lista->info.listaExam;
        while(p != NULL)
        {
            suma+=p->info.nota;
            cont++;
            p = p->sig;
        }
        cout<<"DNI: "<<lista->info.dni<<endl<<"Nombre: "<<lista->info.nombre<<endl<<"Nota Promedio: "<<suma/cont<<endl<<endl;
        lista = lista->sig;
    }
}
...
int procesarConsultas(FILE*aCons,FILE*aLibros,LibVec vL[],int mc[][6],EditorialCons vecEdit[]);
void listadoPto1(LibVec vl[],int mConsul[][6],int cl,FILE*aLibros,EditorialCons ve[]);

int main()
{
    int cantConsul[1000][6];
...
void listadoPto1(LibVec vl[],int mConsul[][6],int cl,FILE*aLibros,EditorialCons ve[])
{
    ...
    for(int i=0;i<cl;i++)
    {
        if(tieneMas20(mConsul,i))
        {
            cout<<vl[i].codLib;
            Libro lib;
            busquedaBinariaA(aLibros,vl[i].codLib,lib);
